...
export default async function handler(req, res) {
    ...
    async function putMethod() {
        try {
            ...
            if (!speakers) {
                res.status(404).send("Error: speakers not found in db.json");
            } else {
            ...
            console.error("error thrown in PUT /api/speakers", e);
            res.status(500).send(`PUT /api/speakers/${id} error: ${e}`);
        }
    }
    ...
    async function postMethod() {
        try {
            ...
            console.error("error thrown in POST /api/speakers", e);
            res.status(500).send(`POST /api/speakers/${id} error: ${e}`);
        }
    }
    ...
    async function deleteMethod() {
        try {
            ...
            console.error("error thrown in DELETE /api/speakers", e);
            res.status(500).send(`DELETE /api/speakers/${id} error: ${e}`);
        }
    }
